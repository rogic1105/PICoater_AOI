...
> 狀態：**規劃完成、待執行**。基準＝前端架構已定（`src/dotnet/AniloxRoll.Monitor/AGENTS.md` 四層 + 協調層五角色）。
> 原則沿用前幾次大重構：**分波 + 每步 build+抽測 + 每波結束是可 merge 乾淨點 + 增量絕不 big-bang**。

## 0. 目標
前端三層已乾淨（①View 元件 / ③State Hub / ④Service）；**只有②協調層還亂**。把協調層整理成五角色
（Coordinator / ControlAdapter / Service / Helper / View，判準見 app AGENTS.md），拆掉 god object，
把純機制搬進 sdk。
...
| `MultiClickDetector` | — | 唯一來源債：app 一份（`AniloxRollForm.cs` panelClicker，MIL 直繪雙擊）vs ImageCanvas 內建一份 | 抽 sdk + ImageCanvas 共用 or 留；**未解** | 3 |
...
- [ ] `BitmapPool` → Imaging/
- [ ] `MultiClickDetector`：**評估**——app 獨立那份可能隨 camReviewMain 死（Wave 2）；若留，抽 sdk + ImageCanvas 改用它（收斂內建重複那份＝唯一來源）
- 每個：移檔+改 namespace+去產品語意+補 XML 註解+csproj 移轉+呼叫端改 using（見 sdk/AGENTS.md 6 步 SOP）
...
- [ ] **①** `OnSettingChanged` dispatcher → **選項1（已定）：不拆掉 dispatcher 本身，拆它順便做的 8 種 feature 細節邏輯。**
  ...
  - 反模式釐清：根 AGENTS.md「OnSettingChanged 勿拆」指的是**結構/鎖別碎**，與本項（搬 feature 邏輯）不衝突。
...
- [ ] **MultiClickDetector 唯一來源債**：app 一份（`AniloxRollForm.cs` panelClicker，MIL 直繪雙擊）vs ImageCanvas 內建一份。動到相關 code 時收斂（抽 sdk 讓 ImageCanvas 共用 or 評估留）。
- 風險：高。每個 god object 一條子分支，先 partial 再抽，每步 build + 完整上機回歸。**絕不一次拆完。**
- **每波結束 doc 同步**（新增步驟）：改完即更新根 `AGENTS.md` 關鍵檔表 + 控制項速查 + 對應 skill（避免文件漂移誤導下次對話）。
