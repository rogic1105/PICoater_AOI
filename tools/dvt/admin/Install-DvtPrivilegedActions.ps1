param(
    ...
    [int]$IoPort = 502,
    [string]$StorageAddress = "192.168.10.20",
    [int]$StoragePort = 445,
...
$storageRoute = @(
    Find-NetRoute -RemoteIPAddress $StorageAddress -ErrorAction Stop) |
    Where-Object {
        $_.CimClass.CimClassName -eq "MSFT_NetRoute"
    } |
    Select-Object -First 1
if (-not $storageRoute) {
    throw "No route was found for storage endpoint $StorageAddress."
}
...
$storageBlockedPrefix = "$StorageAddress/32"
...
`$ErrorActionPreference='Stop'
Get-NetRoute -DestinationPrefix '$storageBlockedPrefix' -PolicyStore ActiveStore -ErrorAction SilentlyContinue | Where-Object { `$_.InterfaceIndex -eq $blackholeInterfaceIndex -and `$_.NextHop -eq '$blackholeNextHop' } | Remove-NetRoute -Confirm:`$false -ErrorAction Stop
New-NetRoute -DestinationPrefix '$storageBlockedPrefix' -InterfaceIndex $blackholeInterfaceIndex -NextHop '$blackholeNextHop' -RouteMetric 1 -PolicyStore ActiveStore -ErrorAction Stop | Out-Null
"@
...
Get-NetRoute `
    -DestinationPrefix $storageBlockedPrefix `
    -PolicyStore ActiveStore `
    -ErrorAction SilentlyContinue |
    Where-Object {
        $_.InterfaceIndex -eq $blackholeInterfaceIndex -and
...
try {
    ...
    if (-not (Test-TcpReachable `
        ...
        -Port $StoragePort `
        -TimeoutMilliseconds 3000)) {
        throw "Storage SMB was not reachable before its route self-test."
    }
    Invoke-FixedTask "PICoater-DVT-Block-Storage"
    $storageBlockedRoute = Get-NetRoute `
        -DestinationPrefix $storageBlockedPrefix `
        -PolicyStore ActiveStore `
        -ErrorAction SilentlyContinue |
        Where-Object {
    ...
    if (-not $storageBlockedRoute) {
        throw "Storage blackhole route self-test did not create the route."
    }
    if (Test-TcpReachable `
        -Address $StorageAddress `
        -Port $StoragePort `
        -TimeoutMilliseconds 1500) {
        throw "Storage blackhole route did not block a new TCP connection."
    }

    Invoke-FixedTask "PICoater-DVT-Unblock-Storage"
    $storageBlockedRoute = Get-NetRoute `
        -DestinationPrefix $storageBlockedPrefix `
        -PolicyStore ActiveStore `
        -ErrorAction SilentlyContinue |
    ...
    if ($storageBlockedRoute) {
        throw "Storage blackhole route self-test did not remove the route."
    }
    if (-not (Test-TcpReachable `
        -Address $StorageAddress `
        -Port $StoragePort `
        -TimeoutMilliseconds 3000)) {
        throw "Storage SMB did not recover after removing the blackhole route."
    }
...
Write-Host "       TestRunner options 10 and 11 no longer need a UAC prompt."
Stop-Transcript | Out-Null
