...
- 座標換算 `pixel↔mm`：已收斂進 sdk `ImageDisplayView.OnCanvasStatus`（唯一來源；live/回顧同用，CanvasInteractionHelper 與 app live 舊路皆已刪，公式走 `PixelMmMapper`）
...
| 顯示/接線改動後驗證、控制項流程追蹤、**方向/座標/單位/轉換鏈**（語意病→轉換點盤點） | `/verify-flows` | UI 行為契約（DVT，持續迴歸）：F1~F8 flow 契約 + 偏序驗證規則 + `[Flow]` log 比對（改 LiveCameraManager/LiveDisplayCoordinator/ImageDisplayView/WaterfallView 後必跑）；含任意控制項 call chain 追蹤法 |
