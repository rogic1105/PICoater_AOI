...
1. **C 標頭宣告** — `sdk/TanukiCv/native/core_cv_api/include/export_c/export_api.h`
   - 加入 `CORE_CV_API` 修飾的函式簽章

2. **C++ 實作** — `sdk/TanukiCv/native/core_cv_api/src/export_api.cpp`
...
3. **C# P/Invoke** — `AniloxRoll.Monitor/Interop/NativeMethods.cs`
   ...
   - **不得** 修改 `sdk/TanukiCv/dotnet/TanukiCv.Core/Core/CoreCVWrapper.cs` 來供 Monitor 使用
