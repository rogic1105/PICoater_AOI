...
| Software row-phase alignment and crop | tag `experiment/software-row-phase-alignment-20260727` | Implemented per-camera row alignment, settings persistence, CFG snapshots, display metadata, and unit coverage. Bench exploration showed that trustworthy cross-card phase still needs a shared Camera Link CC1 clock distributed through AUX hardware; a factory-ready solution would require an industrial pulse splitter and qualified DBHD-15 harnesses. | Paused; not merged. Prefer the accepted capture boundary plus existing crop behavior until hardware synchronization is justified. |
