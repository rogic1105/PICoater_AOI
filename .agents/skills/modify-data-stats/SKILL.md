...
- 資料來源：`LoadConfigForGrabId`（取該 grab 的 #CFG OPS/Pos）+ `LoadImagePathsForGrabId` + `CurveMergeHelper.MergeCurves`（合該 grab 內所有 capture）→ 與 `ReviewStitchCoordinator.UpdateStitchedOverviewChart` 同源 → `chartDataColumn` 與 `chartReviewColumn` 對齊
