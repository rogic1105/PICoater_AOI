...
| `ReviewStitchCoordinator` | Curve and single-grab image IO extracted; alignment policy lives in `FrameTickIndex`; curve display apply, period projection, and display lifecycle remain broad | Continue one responsibility at a time; preserve R2/R3 log contracts at each checkpoint |
