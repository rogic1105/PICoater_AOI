...
| Background capture | `BackgroundProfileRepository` owns all background file IO; capture sampling, camera binding, preview intent, and UI feedback still meet in `AniloxRollForm.Background` | Next extract capture sampling/orchestration into one coordinator; keep native buffer ownership in `AniloxCamera` and view behavior in `LiveDisplayCoordinator` |
