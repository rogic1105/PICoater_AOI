...
| Hardware/output status | `LightConnectionCoordinator` owns light discovery/probe/reconnect/controller lifetime. `AniloxRollForm.HardwareStatus` still sequences IO and storage telemetry; `OutputHealthPresenter` exclusively renders independent output incidents | Continue splitting IO and storage lifecycles only with focused hardware smoke evidence; never move protocol policy into UI presenters |
