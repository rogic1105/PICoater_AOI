# PICoater AOI detailed repository reference

> Detailed maps and lookup tables moved out of root AGENTS.md to stay within the Codex instruction budget. Root and nested AGENTS.md files remain authoritative for rules.

## 詳細 repo 地圖（src / sdk / tools / tests / docs）

```
PICoater_AOI/
├── src/                  ← 應用程式（產品交付；只剩 UI）
│   └── dotnet/AniloxRoll.Monitor/  ← C# WinForms 主應用（C++ 演算法已全數搬入 sdk/TanukiCv/native/tanuki_pipeline）
├── sdk/                  ← 可獨立 split 的 library（純函式庫，無 GUI、無 exe）。**有自己的 `sdk/AGENTS.md`（巢狀，編 sdk 檔時載入；放分層鐵則+元件地圖，隨 split 帶走）**
...
└── .agents/skills/       ← Codex repo skills（依 description 自動觸發，也可用 `$skill-name` 指定）
...
| `Services/FlowTrace.cs` | **[Flow] 資料流跡唯一出口**：使用者動作（ui:/set: intent）＋接線咽喉點＋view 決策（autoFit/lodRebind/clearFrame）＋業務事件（MURA 超標）各一行，每行帶時間戳+執行緒 ID → 落 `Logs\trace-*.log`。**行清單與契約（DVT）唯一來源＝`$verify-flows`**（此處不列舉防過時）。驗證＝執行緒內驗全序、跨執行緒驗因果+完整性；「裝儀器＝同步契約」鐵則見 §Git Workflow 檢查清單 |
...
| `UI/Managers/GlobalMergeCoordinator.cs` | **即時全域合圖協調者**（internal sealed）：擁有 `MultiCameraMerger` 工頭的生命週期（「拼」＝佈局+合併 buffer+每台 merge target；Enable/Disable/RefreshLayout，零 ctor 依賴）。「秀」一律 CPU（顯示鐵則：即時=ImageDisplayView、瀑布=WaterfallView），顯示層要佈局讀 `Merger`（SlotStartsMm/RefOpsMm）。**合圖永遠 Global**（`hb_StitchMode` 寫死 Global，2026-06-13 退場 Vertical） |
| `UI/Managers/LiveDisplayCoordinator.cs` | **即時監控顯示協調者**（2026-06-29 從 LiveCameraManager 提取，internal sealed）：擁有 ImageCanvas/Waterfall/縮圖 panel/狀態 label/選中相機/screen mm-per-px/GPU LOD provider。負責 `ApplyMainDisplayMode`（顯示狀態唯一計算點）、`SetLodMode`、`SwitchMainDisplay`、ImageCanvas cursor/view-range 轉發、Waterfall layout feeding。穩定 UI 依賴走 ctor；變動資料（cameras/settings/lineRate/live 狀態）走 Func，不反向參考整個 LCM；GPU LOD pinned buffer 生命週期委派 `TanukiCv.Core.GpuGrayResizeProvider`（app 注入 `NativeMethods`）。**背景預覽＝view 層靜音鍵**（`_bgPreviewOverride`，唯一真相曝露 `IsBgPreviewActive`、form 唯讀轉發）：是閘門 `ApplyMainDisplayMode` 的輸入之一（BgPreview 分支＝拆瀑布+確保 IC view+設定佈局），`Enter/ExitBackgroundPreview` 只改狀態→呼閘門、不自建拆 view；「存活」policy＝預覽中改設定/相機數變化仍出預覽畫面，退出才生效（2026-07-09 對齊，契約見 `$verify-flows` F8）。 |
...
| 主畫面顯示 | `he_MainDisplay` → `ImageView.MainDisplay` | ImageCanvas | ImageCanvas（即時合圖）/ Waterfall（瀑布合圖）——主畫面永遠合圖、縮圖永遠即時（顯示鐵則見 app 巢狀 AGENTS.md）。ImageCanvas：共用 `ImageDisplayView`〔sdk TanukiCv.Controls；與範例同源唯一來源〕在 camLiveMain 疊 ImageCanvas+各 cam 疊 ThumbStrip 縮圖，吃 `AniloxCamera.OnDisplayFrame` bytes→bitmap，CPU合圖+mm overlay+zoom+雙三擊+LOD。滾輪縮放＝ImageCanvas 自理（app 無全域訊息濾鏡）。**bin↔主畫面連動**：`ImageDisplayView.ViewRangeMmChanged(left,right,top,bot)`→`LiveCameraManager.OnImageViewRange`→`OnLiveViewRange`事件→form `ApplyLiveViewRange`：欄/overview用X、列用Y zoom 同步（列需 `LiveCameraManager.RowPitchMm` 餵真 row pitch；overview 用 `LiveViewRangeProvider` 沿用同範圍→500ms 重畫不閃）。**TODO：縮圖多相機同步刷 / 關底層 MIL / 回顧側 CanvasInteractionHelper 視野計算收斂進 sdk / sample 重用曲線圖+閾值線選用 / app live 實體化 LOD/flip UI**）|
...
可重複工作流程統一放 `.agents/skills/`，`docs/dev/` 放大型參考文件。
