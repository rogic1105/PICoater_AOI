...
# 依賴先跑過 configure_network_share.ps1（共用資料夾已存在）
...
# ── 2. 授予 Guest 共用權限（SMB） ────────────
Write-Host "[2/4] 授予 Guest SMB 共用權限..."
...
# ── 3. NTFS：授予 Guest Modify ───────────────
Write-Host "[3/4] 授予 Guest NTFS 寫入權限..."
$folder = $cfg.AniloxRoot
$acl = Get-Acl $folder
...
# ── 4. 修正 Guest 網路登入權利（關鍵！Windows 預設 Guest 被拒絕網路登入）
Write-Host "[4/4] 修正 Guest 網路登入權利（secedit）..."
...
Write-Host "        接著請在檢測機執行 setup.bat"
Write-Host ("        測試：檢測機 Win+R 輸入 \\" + $cfg.IpAddress + "\" + $cfg.ShareName + " 應直接開啟")
