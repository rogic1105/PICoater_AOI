# AOI SDK (Automated Optical Inspection SDK)

本 SDK 是一個高效能的電腦視覺開發框架。底層採用 CUDA (Core CV) 進行 GPU 加速，並提供統一的測試框架 (Framework) 與工具庫 (Cpp Utils)。
...
int main() {
    // 使用 framework 的啟動器，它會自動處理路徑與例外
    return framework::RunTestBootstrap("My Custom Test Suite", RunMyTest);
}
