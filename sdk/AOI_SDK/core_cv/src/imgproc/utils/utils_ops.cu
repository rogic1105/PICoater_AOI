// AOI_SDK\core_cv\src\imgproc\utils\utils_ops.cu
...
namespace core {
    ...
    // 正規化: Float -> Uint8 (MinMax 0-255)
    void normalize_minmax_f32_u8_gpu(const float* d_in, uint8_t* d_out, int N, cudaStream_t s) {
        // 1. 使用 Thrust 找出最大值與最小值
        // 建立 Thrust Device Pointer (不會觸發複製，只是包裝指標)
        thrust::device_ptr<const float> d_ptr(d_in);

        // 執行 minmax_element
        // 使用 thrust::cuda::par.on(s) 確保在指定的 Stream 上執行
        auto result = thrust::minmax_element(thrust::cuda::par.on(s), d_ptr, d_ptr + N);

        // 2. 將結果從 Device 傳回 Host
        float min_val, max_val;
        // result.first 和 second 是 device iterator，需要取出其指向的值
        // 使用 Async 複製以配合 Stream
        ...
        // [重要] 這裡必須同步 Stream，因為接下來的 Kernel Launch 需要用到 CPU 端的 min_val/max_val
